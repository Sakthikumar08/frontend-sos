...
const Hocmatches = () => {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/hocmatches');
  ...
  const handleAddMatch = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/hocmatches', newMatch);
