...
const Ballplayers = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/ballplayers')
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:5000/api/ballplayers', { ...newPlayer, stats: playerStats })
      .then(response => {
  ...
  const handleSaveStats = () => {
    ...
    axios.put(`http://localhost:5000/api/ballplayers/${selectedPlayer._id}/stats`, playerStats)
      .then(response => {
  ...
  const handleDeletePlayer = () => {
    axios.delete(`http://localhost:5000/api/ballplayers/${selectedPlayer._id}`)
      .then(() => {
